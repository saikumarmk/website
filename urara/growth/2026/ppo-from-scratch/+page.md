...
**Intuition:** If ratio greater than 1+ε or less than 1-ε, clip it. Don't let policy change too much.

## Implementation Checklist

### Core Components

✅ **Policy network** ($\pi_\theta$): Output action probabilities  
✅ **Value network** ($V_\phi$): Estimate state values  
✅ **Advantage computation** (GAE with $\lambda = 0.95$)  
✅ **Probability ratio**: $r_t = \pi_{\text{new}} / \pi_{\text{old}}$  
✅ **Clipped objective**: $\min(r \cdot A, \text{clip}(r, 1-\epsilon, 1+\epsilon) \cdot A)$  
...
- **clip_eps (ε):** 0.1–0.3 (default 0.2)
- **GAE lambda (λ):** 0.9–0.99 (default 0.95)
- **Learning rate:** 3e-4 (tune with lr schedule)
- **Minibatch size:** 64–256
- **Number of epochs:** 3–10 (watch for overfitting)
...
❌ **Ignoring clip fraction:** If it's 0 or 1, something's wrong.
